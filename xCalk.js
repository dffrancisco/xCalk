    ...
    function create(params) {
        ...
        const ax = {
            ...
            main() {
                ...
                div.addEventListener('keydown', this.enter)
            ...
            setButtons() {
                ...
                let btns = ['AC', '«', '%', '÷', '7', '8', '9', 'x', '4', '5', '6', '-', '1', '2', '3', '+', 'p', '0', ',', '=']
                ...
                for (let i in btns) {
                    ...
                    //     //console.log(btns[i]);
                    //     console.log(this.formatValor(btns[i]));
            ...
            enter(e) {
                ...
                if (chrome === 189 || mozila === 173 || e.keyCode === 109)
                    ax.operador("-");
                if (
                    (chrome === 187 && e.shiftKey) ||
                    (mozila === 61 && e.shiftKey) ||
                    e.keyCode === 107
                )
                    ax.operador("+");
                ...
                if (
                    ...
                    ax.valorCalc(",");
                if (e.keyCode === 8) ax.backSpace();
                if (e.keyCode === 67) ax.clearAll();
                // if (e.keyCode === 40) _cal("P");
            },
            operador(value) {
                if (this.soma == 0)
                // this.resultado.push({ op: "", value: this.ultimoValor });
                    this.setHistory('', this.ultimoValor)
                else
                // this.resultado.push({ op: value, value: this.ultimoValor });
            ...
            calcular() {
                ...
                //  let pnResul = document.getElementById("pnResul");
                setTimeout(() => {
                    this.elHistory.scrollBy(0, 9999999999);
                }, 50);

                //  console.log(this.ultimoValor, 'ultimoValor')
            },
            percento() {
                if (this.op === "x")
                    this.soma =
                    (parseFloat(this.soma) * parseFloat(this.ultimoValor)) / 100;

                if (this.op === "+")
                    this.soma =
                    (parseFloat(this.soma) * parseFloat(this.display)) / 100 +
                ...
                if (this.op === "÷")
                    this.soma =
                    (parseFloat(this.soma) * parseFloat(this.display)) /
                    100 /
                    parseFloat(this.soma);

                // this.resultado.push({ op: this.op + "%", value: this.ultimoValor });
                // this.resultado.push({ op: "=", value: this.soma });
                // this.resultado.push({ op: "===================", value: "" });
            ...
            igual() {
                // this.resultado.push({ op: this.op, value: this.display });
            ...
            setHistory(op, value) {
                ...
                div.appendChild(span)
                this.elHistory.appendChild(div)
            }
        }
        ax.main()
        console.log(ax);
